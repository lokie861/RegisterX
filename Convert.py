...
# def plc_mapper(series: str, plc_address: str, addr_type: str) -> tuple[str, str]:
#     global PLC_MAPPINGS
...
#     maps = PLC_MAPPINGS.get(series, {}).get(reg_type, {}).get(addr_type, {})
#     if not maps:
...
def sv2(plc_address: str, addr_type: str) -> tuple[str, str]:
    ...
    maps = PLC_MAPPINGS["SV2"].get(reg_type, {}).get(addr_type, {})
    if not maps:
        return ("Unsupported Register Type", "Unsupported Register Type")
...
def sa2(plc_address: str, addr_type: str) -> tuple[str, str]:
    ...
    maps = PLC_MAPPINGS["SA2"].get(reg_type, {}).get(addr_type, {})
    if not maps:
        return ("Unsupported Register Type", "Unsupported Register Type")
...
def ss2(plc_address: str, addr_type: str) -> tuple[str, str]:
    ...
    maps = PLC_MAPPINGS["SS2"].get(reg_type, {}).get(addr_type, {})
    if not maps:
        return ("Unsupported Register Type", "Unsupported Register Type")
...
def as_series(plc_address: str, addr_type: str) -> tuple[str, str]:
    ...
    possible_types = sorted(PLC_MAPPINGS["AS"].keys(), key=len, reverse=True)
    ...
    maps = PLC_MAPPINGS["AS"].get(reg_type, {}).get(addr_type, {})
    if not maps:
        return ("Unsupported Register Type", "Unsupported Register Type")
